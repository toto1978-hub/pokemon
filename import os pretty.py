...
def make_hero_html(top_result: Dict[str, Any]) -> str:
    return f"""
    <div class="hero-match-card">
        <div class="hero-caption">당신과 가장 잘 어울리는 포켓몬</div>
        <div class="hero-name">{top_result["name"]}</div>
        <div class="hero-ring-wrap">
            {ring_svg(top_result["score"])}
        </div>
        <div class="hero-badge-row">{score_badge(top_result["score"])}</div>
        <div class="hero-type-row">{render_type_badges(top_result["types"])}</div>
        <div class="hero-keyword-row">{render_keyword_badges(top_result["keywords"])}</div>
        <p class="hero-desc">{top_result["explanation"]}</p>
    </div>
    """


def make_top1_html(top_result: Dict[str, Any]) -> str:
    b = top_result["breakdown"]
    return f"""
    <div class="top-card">
        <div class="top-head">
            {img_tag_or_fallback(top_result, "top-thumb")}
            <div class="top-meta">
                <div class="top-label">1위 추천 포켓몬</div>
                <h2>{top_result["name"]}</h2>
                <div class="type-row">{render_type_badges(top_result["types"])}</div>
                <div class="keyword-row">{render_keyword_badges(top_result["keywords"])}</div>
            </div>
        </div>
        <p class="top-desc">{top_result["explanation"]}</p>
        <div class="bar-group">
        ...
        <div class="rank-title">
            <div class="rank-left">
                ...
                <div class="rank-name-wrap">
                    ...
                    <div class="type-row compact">{render_type_badges(result["types"])}</div>
                </div>
            </div>
...
def predict(image: np.ndarray):
    ...
    try:
        ...
        hero_html = make_hero_html(top_result)
        top1_image = top_result["image_path"] if os.path.exists(top_result["image_path"]) else None
        detail_html = make_top1_html(top_result)
        rank_html = make_all_rank_html(results)

        return annotated_rgb, feature_html, hero_html, top1_image, detail_html, rank_html

    except Exception as e:
        err = f"<div class='empty-box'>오류: {str(e)}</div>"
        return None, err, err, None, err, err
...
.hero-banner h1 { margin: 0 0 8px 0; font-size: 30px; }
.hero-banner p { margin: 0; color: #5b4a2f; line-height: 1.6; }

.panel, .feature-card, .top-card, .rank-card, .hero-match-card, .empty-box {
    border-radius: 20px;
...
.feature-card, .top-card, .rank-card, .hero-match-card, .empty-box {
    padding: 18px;
}
...
.type-normal { background: #f3f4f6; color: #374151; border-color: #d1d5db; }
.type-electric { background: #fef3c7; color: #92400e; border-color: #fcd34d; }
.type-fire { background: #fee2e2; color: #b91c1c; border-color: #fca5a5; }
.type-psychic { background: #f5d0fe; color: #a21caf; border-color: #e879f9; }
.type-fairy { background: #fce7f3; color: #be185d; border-color: #f9a8d4; }
.type-dark { background: #e5e7eb; color: #111827; border-color: #9ca3af; }
.type-default { background: #eef2ff; color: #3730a3; border-color: #c7d2fe; }
...
.face-preview-box, .pokemon-preview-box {
    width: 100%;
    min-height: 250px;
    border-radius: 20px;
    border: 1px solid #e5e7eb;
...
.face-preview-title, .pokemon-preview-title {
    font-size: 14px;
    color: #6b7280;
    margin-bottom: 10px;
    text-align: center;
}
.pokemon-preview-box img {
...
with gr.Blocks(css=CUSTOM_CSS, title="사람 얼굴 ↔ 포켓몬 매칭 데모") as demo:
    ...
    <div class="hero-banner">
        ...
        <p>
            얼굴 사진을 업로드하면 얼굴형, 눈매, 표정, 분위기를 분석한 뒤
            가장 잘 어울리는 포켓몬을 히어로 섹션으로 강조해 보여줍니다.
        </p>
    </div>
    """)
    ...
    with gr.Row():
        with gr.Column(scale=1):
            gr.HTML("<div class='face-preview-title'>입력 얼굴 분석 결과</div>")
            hero_face_image = gr.Image(label=None, show_label=False)
        with gr.Column(scale=1):
            hero_html = gr.HTML("<div class='empty-box'>매칭 히어로 카드가 여기에 표시됩니다.</div>")
        with gr.Column(scale=1):
            gr.HTML("<div class='pokemon-preview-title'>1위 포켓몬 썸네일</div>")
            top1_image = gr.Image(label=None, show_label=False)

    detail_html = gr.HTML("<div class='empty-box'>1위 포켓몬 상세가 여기에 표시됩니다.</div>")
    rank_html = gr.HTML("<div class='empty-box'>Top 3 결과 카드가 여기에 표시됩니다.</div>")

    run_btn.click(
        fn=predict,
        inputs=input_image,
        outputs=[hero_face_image, feature_html, hero_html, top1_image, detail_html, rank_html]
    )
